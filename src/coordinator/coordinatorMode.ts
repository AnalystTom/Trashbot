...
export function isCoordinatorMode(): boolean {
  if (feature('COORDINATOR_MODE')) {
    return isEnvTruthy(process.env.TRASH_BOT_COORDINATOR_MODE)
  }
  return false
}
...
export function matchSessionMode(
  ...
  if (sessionIsCoordinator) {
    process.env.TRASH_BOT_COORDINATOR_MODE = '1'
  } else {
    delete process.env.TRASH_BOT_COORDINATOR_MODE
  }
...
export function getCoordinatorUserContext(
  ...
  const workerTools = isEnvTruthy(process.env.TRASH_BOT_SIMPLE)
...
export function getCoordinatorSystemPrompt(): string {
  const workerCapabilities = isEnvTruthy(process.env.TRASH_BOT_SIMPLE)
    ? 'Workers have access to Bash, Read, and Edit tools, plus MCP tools from configured MCP servers.'
    : 'Workers have access to standard tools, MCP tools from configured MCP servers, and project skills via the Skill tool. Delegate skill invocations (e.g. /commit, /verify) to workers.'

  return `You are Trash Bot, an AI assistant that orchestrates software engineering tasks across multiple workers.
...
2. Precise git operation: "Create a new branch from main called 'fix/session-expiry'. Cherry-pick only commit abc123 onto it. Push and create a draft PR targeting main. Add anthropics/trash_bot as reviewer. Report the PR URL."
