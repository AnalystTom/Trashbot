...
async function handleSpawnSplitPane(
  ...
  // Build the command to spawn Trash Bot with teammate identity
  // Note: We spawn without a prompt - initial instructions are sent via mailbox
  const binaryPath = getTeammateCommand()

  // Build teammate identity CLI args (replaces TRASH_BOT_* env vars)
  const teammateArgs = [
  ...
  // Includes CLAUDECODE, TRASH_BOT_EXPERIMENTAL_AGENT_TEAMS, and API provider vars.
...
/**
 * Handle spawn operation - creates a new Trash Bot instance.
