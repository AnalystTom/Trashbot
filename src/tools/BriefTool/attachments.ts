...
export async function resolveAttachments(
  ...
  if (feature('BRIDGE_MODE')) {
    ...
    // REPL does, at main.tsx init). TRASH_BOT_BRIEF_UPLOAD lets a host that
    // runs the CLI as a subprocess opt in — e.g. the cowork desktop bridge,
    // which already passes TRASH_BOT_OAUTH_TOKEN for auth.
    const shouldUpload =
      uploadCtx.replBridgeEnabled ||
      isEnvTruthy(process.env.TRASH_BOT_BRIEF_UPLOAD)
