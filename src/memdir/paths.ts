 ...
 *   1. TRASH_BOT_DISABLE_AUTO_MEMORY env var (1/true → OFF, 0/false → ON)
 *   2. TRASH_BOT_SIMPLE (--bare) → OFF
 *   3. CCR without persistent storage → OFF (no TRASH_BOT_REMOTE_MEMORY_DIR)
 *   4. autoMemoryEnabled in settings.json (supports project-level opt-out)
 *   5. Default: enabled
 */
export function isAutoMemoryEnabled(): boolean {
  const envVal = process.env.TRASH_BOT_DISABLE_AUTO_MEMORY
  ...
  if (
    ...
    !process.env.TRASH_BOT_REMOTE_MEMORY_DIR
  ) {
    return false
  }
 ...
 * Resolution order:
 *   1. TRASH_BOT_REMOTE_MEMORY_DIR env var (explicit override, set in CCR)
 *   2. ~/.claude (default config home)
 */
export function getMemoryBaseDir(): string {
  if (process.env.TRASH_BOT_REMOTE_MEMORY_DIR) {
    return process.env.TRASH_BOT_REMOTE_MEMORY_DIR
  }
 ...
 * same repo share one auto-memory directory (anthropics/trash_bot#24382).
 */
