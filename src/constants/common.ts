...
export function getLocalISODate(): string {
  ...
  if (process.env.TRASH_BOT_OVERRIDE_DATE) {
    return process.env.TRASH_BOT_OVERRIDE_DATE
  }
...
export function getLocalMonthYear(): string {
  const date = process.env.TRASH_BOT_OVERRIDE_DATE
    ? new Date(process.env.TRASH_BOT_OVERRIDE_DATE)
    : new Date()
