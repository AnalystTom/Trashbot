...
const DEFAULT_PREFIX = `You are Trash Bot, Anthropic's official CLI for Claude.`
const AGENT_SDK_TRASH_BOT_PRESET_PREFIX = `You are Trash Bot, Anthropic's official CLI for Claude, running within the Claude Agent SDK.`
const AGENT_SDK_PREFIX = `You are a Claude agent, built on Anthropic's Claude Agent SDK.`

const CLI_SYSPROMPT_PREFIX_VALUES = [
  DEFAULT_PREFIX,
  AGENT_SDK_TRASH_BOT_PRESET_PREFIX,
  AGENT_SDK_PREFIX,
] as const
...
export function getCLISyspromptPrefix(options?: {
  ...
  if (options?.isNonInteractive) {
    if (options.hasAppendSystemPrompt) {
      return AGENT_SDK_TRASH_BOT_PRESET_PREFIX
    }
...
function isAttributionHeaderEnabled(): boolean {
  if (isEnvDefinedFalsy(process.env.TRASH_BOT_ATTRIBUTION_HEADER)) {
    return false
  }
...
export function getAttributionHeader(fingerprint: string): string {
  ...
  const entrypoint = process.env.TRASH_BOT_ENTRYPOINT ?? 'unknown'
