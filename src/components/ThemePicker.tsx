...
export function ThemePicker(t0) {
  ...
  const t22 = colorModuleUnavailableReason === "env" ? `Syntax highlighting disabled (via TRASH_BOT_SYNTAX_HIGHLIGHT=${process.env.TRASH_BOT_SYNTAX_HIGHLIGHT})` : syntaxHighlightingDisabled ? `Syntax highlighting disabled (${syntaxToggleShortcut} to enable)` : syntaxTheme ? `Syntax theme: ${syntaxTheme.theme}${syntaxTheme.source ? ` (from ${syntaxTheme.source})` : ""} (${syntaxToggleShortcut} to disable)` : `Syntax highlighting enabled (${syntaxToggleShortcut} to disable)`;
  let t23;
  if ($[37] !== t22) {
