...
export async function refreshOAuthToken(
  ...
  try {
    ...
    // TRASH_BOT_OAUTH_REFRESH_TOKEN re-login path: installOAuthTokens runs
...
export async function populateOAuthAccountInfoIfNeeded(): Promise<boolean> {
  ...
  const envAccountUuid = process.env.TRASH_BOT_ACCOUNT_UUID
  const envUserEmail = process.env.TRASH_BOT_USER_EMAIL
  const envOrganizationUuid = process.env.TRASH_BOT_ORGANIZATION_UUID
