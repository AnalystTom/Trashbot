...
// OAuth credentials as Trash Bot.  The endpoint uses conversation_engine
...
export function isVoiceStreamAvailable(): boolean {
  // voice_stream uses the same OAuth as Trash Bot — available when the
...
export async function connectVoiceStream(
  ...
  // non-browser clients (anthropics/trash_bot#34094). Same private-api
  ...
  ws.on('unexpected-response', (req: ClientRequest, res: IncomingMessage) => {
    ...
    // successful 101 Switching Protocols response (anthropics/trash_bot#40510).
