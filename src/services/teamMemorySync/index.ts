 ...
 *   GET  /api/trash_bot/team_memory?repo={owner/repo}            → TeamMemoryData (includes entryChecksums)
 *   GET  /api/trash_bot/team_memory?repo={owner/repo}&view=hashes → metadata + entryChecksums only (no entry bodies)
 *   PUT  /api/trash_bot/team_memory?repo={owner/repo}            → upload entries (upsert semantics)
...
// GB-tunable per-org (trash_bot_team_memory_limits), so any compile-time
...
function getTeamMemorySyncEndpoint(repoSlug: string): string {
  ...
  return `${baseUrl}/api/trash_bot/team_memory?repo=${encodeURIComponent(repoSlug)}`
}
...
async function readLocalTeamMemory(maxEntries: number | null): Promise<{
  ...
  // trash_bot_team_memory_limits; we have no way to know it in advance.
