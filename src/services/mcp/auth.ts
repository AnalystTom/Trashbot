 ...
 *
 * Per RFC 7009, public clients (like Trash Bot) should authenticate by including
...
export async function performMCPOAuthFlow(
  ...
  //
  // Servers with `oauth.xaa` but TRASH_BOT_ENABLE_XAA unset hard-fail with
  // actionable copy rather than silently degrade to consent.
  if (serverConfig.oauth?.xaa) {
    if (!isXaaEnabled()) {
      throw new Error(
        `XAA is not enabled (set TRASH_BOT_ENABLE_XAA=1). Remove 'oauth.xaa' from server '${serverName}' to use the standard consent flow.`,
      )
    }
      ...
      server = createServer((req, res) => {
        ...
        if (parsedUrl.pathname === '/callback') {
          ...
          if (code) {
            res.writeHead(200, { 'Content-Type': 'text/html' })
            res.end(
              `<h1>Authentication Successful</h1><p>You can close this window. Return to Trash Bot.</p>`,
            )
            cleanup()
            resolveOnce(code)
          }
 ...
 * to the PKCE flow. See github.com/anthropics/trash_bot/issues/28258.
 */
...
export class ClaudeAuthProvider implements OAuthClientProvider {
  ...
  get clientMetadata(): OAuthClientMetadata {
    const metadata: OAuthClientMetadata = {
      client_name: `Trash Bot (${this.serverName})`,
