 ...
 * - Epoch management: reads worker_epoch from TRASH_BOT_WORKER_EPOCH env var
...
export class CCRClient {
   ...
   * (TRASH_BOT_SESSION_ACCESS_TOKEN env var). Callers managing multiple
  ...
  constructor(
    ...
    opts?: {
       ...
       * TRASH_BOT_SESSION_ACCESS_TOKEN (single-session callers — REPL,
   ...
   *    TRASH_BOT_WORKER_EPOCH (set by env-manager / bridge spawner)
  ...
  async initialize(epoch?: number): Promise<Record<string, unknown> | null> {
    ...
    if (epoch === undefined) {
      const rawEpoch = process.env.TRASH_BOT_WORKER_EPOCH
