...
export async function authLogin({
  ...
  const envRefreshToken = process.env.TRASH_BOT_OAUTH_REFRESH_TOKEN
  if (envRefreshToken) {
    const envScopes = process.env.TRASH_BOT_OAUTH_SCOPES
    if (!envScopes) {
      process.stderr.write(
        'TRASH_BOT_OAUTH_SCOPES is required when using TRASH_BOT_OAUTH_REFRESH_TOKEN.\n' +
          'Set it to the space-separated scopes the refresh token was issued with\n' +
          '(e.g. "user:inference" or "user:profile user:inference user:sessions:trash_bot user:mcp_servers").\n',
      )
      process.exit(1)
    }
