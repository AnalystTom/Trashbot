...
/**
 * Trash Bot parses history for pasted content references to match back to
...
export function addToHistory(command: HistoryEntry | string): void {
  // Skip history when running in a tmux session spawned by Trash Bot's Tungsten tool.
  // This prevents verification/test sessions from polluting the user's real command history.
  if (isEnvTruthy(process.env.TRASH_BOT_SKIP_PROMPT_HISTORY)) {
    return
  }
