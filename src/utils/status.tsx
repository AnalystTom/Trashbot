...
export function buildAPIProviderProperties(): Property[] {
  ...
  if (apiProvider === 'firstParty') {
    ...
    if (isEnvTruthy(process.env.TRASH_BOT_SKIP_BEDROCK_AUTH)) {
      properties.push({
  ...
  if (mtlsConfig) {
    if (mtlsConfig.cert && process.env.TRASH_BOT_CLIENT_CERT) {
      properties.push({
        label: 'mTLS client cert',
        value: process.env.TRASH_BOT_CLIENT_CERT
      });
    }
    if (mtlsConfig.key && process.env.TRASH_BOT_CLIENT_KEY) {
      properties.push({
        label: 'mTLS client key',
        value: process.env.TRASH_BOT_CLIENT_KEY
      });
    }
  }
  return properties;
