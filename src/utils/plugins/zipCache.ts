 ...
 * When TRASH_BOT_PLUGIN_USE_ZIP_CACHE is enabled and TRASH_BOT_PLUGIN_CACHE_DIR
...
export function isPluginZipCacheEnabled(): boolean {
  return isEnvTruthy(process.env.TRASH_BOT_PLUGIN_USE_ZIP_CACHE)
}

/**
 * Get the path to the zip cache directory.
 * Requires TRASH_BOT_PLUGIN_CACHE_DIR to be set.
...
export function getPluginZipCachePath(): string | undefined {
  ...
  const dir = process.env.TRASH_BOT_PLUGIN_CACHE_DIR
...
async function collectFilesForZip(
  ...
  // See also: markdownConfigLoader.ts getFileIdentity, anthropics/trash_bot#13893
  try {
