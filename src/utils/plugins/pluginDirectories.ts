 ...
 * - CLI flag: --cowork
 * - Environment variable: TRASH_BOT_USE_COWORK_PLUGINS
 *
 * The base directory can be overridden via TRASH_BOT_PLUGIN_CACHE_DIR.
 */
 ...
 * 2. Environment variable TRASH_BOT_USE_COWORK_PLUGINS
 * 3. Default: 'plugins'
 */
function getPluginsDirectoryName(): string {
  ...
  if (isEnvTruthy(process.env.TRASH_BOT_USE_COWORK_PLUGINS)) {
    return COWORK_PLUGINS_DIR
  }
  return PLUGINS_DIR
}
 ...
 *
 * Priority:
 * 1. TRASH_BOT_PLUGIN_CACHE_DIR env var (explicit override)
 * 2. Default: ~/.claude/plugins or ~/.claude/cowork_plugins
 */
export function getPluginsDirectory(): string {
  // expandTilde: when TRASH_BOT_PLUGIN_CACHE_DIR is set via settings.json
  ...
  const envOverride = process.env.TRASH_BOT_PLUGIN_CACHE_DIR
  if (envOverride) {
 ...
 * image and point TRASH_BOT_PLUGIN_SEED_DIR at it. CC will use it as a
...
export function getPluginSeedDirs(): string[] {
  ...
  const raw = process.env.TRASH_BOT_PLUGIN_SEED_DIR
  if (!raw) return []
