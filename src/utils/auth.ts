...
function isManagedOAuthContext(): boolean {
  return (
    isEnvTruthy(process.env.TRASH_BOT_REMOTE) ||
    process.env.TRASH_BOT_ENTRYPOINT === 'claude-desktop'
  )
}
...
export function isAnthropicAuthEnabled(): boolean {
  ...
  // local auth-injecting proxy. The launcher sets TRASH_BOT_OAUTH_TOKEN as a
  ...
  if (process.env.ANTHROPIC_UNIX_SOCKET) {
    return !!process.env.TRASH_BOT_OAUTH_TOKEN
  }
  ...
  const hasExternalAuthToken =
    ...
    apiKeyHelper ||
    process.env.TRASH_BOT_API_KEY_FILE_DESCRIPTOR
...
export function getAuthTokenSource() {
  ...
  if (process.env.TRASH_BOT_OAUTH_TOKEN) {
    return { source: 'TRASH_BOT_OAUTH_TOKEN' as const, hasToken: true }
  }
  ...
  if (oauthTokenFromFd) {
    ...
    if (process.env.TRASH_BOT_OAUTH_TOKEN_FILE_DESCRIPTOR) {
      return {
        source: 'TRASH_BOT_OAUTH_TOKEN_FILE_DESCRIPTOR' as const,
        hasToken: true,
      }
    }
    return {
...
export function getAnthropicApiKeyWithSource(
  ...
  if (isEnvTruthy(process.env.CI) || process.env.NODE_ENV === 'test') {
    ...
    if (
      !apiKeyEnv &&
      !process.env.TRASH_BOT_OAUTH_TOKEN &&
      !process.env.TRASH_BOT_OAUTH_TOKEN_FILE_DESCRIPTOR
    ) {
      throw new Error(
        'ANTHROPIC_API_KEY or TRASH_BOT_OAUTH_TOKEN env var is required',
      )
    }
 ...
 * Uses TRASH_BOT_API_KEY_HELPER_TTL_MS env var if set and valid,
 * otherwise defaults to 5 minutes
 */
export function calculateApiKeyHelperTTL(): number {
  const envTtl = process.env.TRASH_BOT_API_KEY_HELPER_TTL_MS
  ...
  if (envTtl) {
    ...
    logForDebugging(
      `Found TRASH_BOT_API_KEY_HELPER_TTL_MS env var, but it was not a valid number. Got ${envTtl}`,
      { level: 'error' },
    )
  }
...
export const getClaudeAIOAuthTokens = memoize((): OAuthTokens | null => {
  ...
  if (process.env.TRASH_BOT_OAUTH_TOKEN) {
    ...
    return {
      accessToken: process.env.TRASH_BOT_OAUTH_TOKEN,
      refreshToken: null,
      expiresAt: null,
...
export function getOtelHeadersFromHelper(): Record<string, string> {
  ...
  const debounceMs = parseInt(
    process.env.TRASH_BOT_OTEL_HEADERS_HELPER_DEBOUNCE_MS ||
...
export function getAccountInformation() {
  ...
  if (
    authTokenSource === 'TRASH_BOT_OAUTH_TOKEN' ||
    authTokenSource === 'TRASH_BOT_OAUTH_TOKEN_FILE_DESCRIPTOR'
  ) {
...
export async function validateForceLoginOrg(): Promise<OrgValidationResult> {
  ...
  const isEnvVarToken =
    source === 'TRASH_BOT_OAUTH_TOKEN' ||
    source === 'TRASH_BOT_OAUTH_TOKEN_FILE_DESCRIPTOR'
  ...
  if (isEnvVarToken) {
    const envVarName =
      source === 'TRASH_BOT_OAUTH_TOKEN'
        ? 'TRASH_BOT_OAUTH_TOKEN'
        : 'TRASH_BOT_OAUTH_TOKEN_FILE_DESCRIPTOR'
    return {
      valid: false,
      message:
