 ...
 * pipeline from user input to first token arrival. Enable by setting TRASH_BOT_PROFILE_QUERY=1
 *
...
const ENABLED = isEnvTruthy(process.env.TRASH_BOT_PROFILE_QUERY)
...
function getQueryProfileReport(): string {
  if (!ENABLED) {
    return 'Query profiling not enabled (set TRASH_BOT_PROFILE_QUERY=1)'
  }
