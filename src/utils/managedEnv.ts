 ...
 * TRASH_BOT_PROVIDER_MANAGED_BY_HOST in spawn env), strip
...
function withoutHostManagedProviderVars(
  ...
  if (!env) return {}
  if (!isEnvTruthy(process.env.TRASH_BOT_PROVIDER_MANAGED_BY_HOST)) {
    return env
  }
...
export function applySafeConfigEnvironmentVariables(): void {
  ...
  // flagSettings env applied. Eligibility reads TRASH_BOT_USE_BEDROCK,
  ...
  // when TRASH_BOT_PROVIDER_MANAGED_BY_HOST is set.
 ...
 * TRASH_BOT_PROVIDER_MANAGED_BY_HOST is set — see filterSettingsEnv) and
