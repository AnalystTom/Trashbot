 ...
 * See: https://github.com/anthropics/trash_bot/issues/13893
 *
...
export const loadMarkdownFilesForSubdir = memoize(
  async function (
    ...
    // (anthropics/trash_bot#29599, #28182, #26992).
    //
 ...
 * 3. Can be explicitly enabled via TRASH_BOT_USE_NATIVE_FILE_SEARCH env var
 *
 * Symlink handling:
...
> {
  ...
  // - Fallback: native Node.js (when TRASH_BOT_USE_NATIVE_FILE_SEARCH is set)
  //
  // Why both? Ripgrep has poor startup performance in native builds.
  const useNative = isEnvTruthy(process.env.TRASH_BOT_USE_NATIVE_FILE_SEARCH)
