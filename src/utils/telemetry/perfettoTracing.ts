/**
 * Perfetto Tracing for Trash Bot (Ant-only)
 *
 ...
 *
 * Usage:
 * 1. Enable via TRASH_BOT_PERFETTO_TRACE=1 or TRASH_BOT_PERFETTO_TRACE=<path>
 * 2. Optionally set TRASH_BOT_PERFETTO_WRITE_INTERVAL_S=<positive integer> to write the
 *    trace file periodically (default: write only on exit).
 * 3. Run Trash Bot normally
...
export function initializePerfettoTracing(): void {
  const envValue = process.env.TRASH_BOT_PERFETTO_TRACE
  logForDebugging(
  ...
  if (feature('PERFETTO_TRACING')) {
    ...
    // Start periodic full-trace write if TRASH_BOT_PERFETTO_WRITE_INTERVAL_S is a positive integer
    const intervalSec = parseInt(
      process.env.TRASH_BOT_PERFETTO_WRITE_INTERVAL_S ?? '',
      10,
    )
    if (intervalSec > 0) {
