...
export const findGitBashPath = memoize((): string => {
  if (process.env.TRASH_BOT_GIT_BASH_PATH) {
    if (checkPathExists(process.env.TRASH_BOT_GIT_BASH_PATH)) {
      return process.env.TRASH_BOT_GIT_BASH_PATH
    }
    // biome-ignore lint/suspicious/noConsole:: intentional console output
    console.error(
      `Trash Bot was unable to find TRASH_BOT_GIT_BASH_PATH path "${process.env.TRASH_BOT_GIT_BASH_PATH}"`,
    )
  ...
  // biome-ignore lint/suspicious/noConsole:: intentional console output
  console.error(
    'Trash Bot on Windows requires git-bash (https://git-scm.com/downloads/win). If installed but not in PATH, set environment variable pointing to your bash.exe, similar to: TRASH_BOT_GIT_BASH_PATH=C:\\Program Files\\Git\\bin\\bash.exe',
  )
  // eslint-disable-next-line custom-rules/no-process-exit
  process.exit(1)
})
