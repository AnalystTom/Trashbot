...
function getTokenFromFileDescriptor(): string | null {
  ...
  const fdEnv = process.env.TRASH_BOT_WEBSOCKET_AUTH_FILE_DESCRIPTOR
  if (!fdEnv) {
  ...
  if (Number.isNaN(fd)) {
    logForDebugging(
      `TRASH_BOT_WEBSOCKET_AUTH_FILE_DESCRIPTOR must be a valid file descriptor number, got: ${fdEnv}`,
      { level: 'error' },
    )
 ...
 *
 * Priority order:
 *  1. Environment variable (TRASH_BOT_SESSION_ACCESS_TOKEN) — set at spawn time,
 ...
 *  2. File descriptor (legacy path) — TRASH_BOT_WEBSOCKET_AUTH_FILE_DESCRIPTOR,
...
export function getSessionIngressAuthToken(): string | null {
  ...
  const envToken = process.env.TRASH_BOT_SESSION_ACCESS_TOKEN
  if (envToken) {
    return envToken
  }
...
export function getSessionIngressAuthHeaders(): Record<string, string> {
  ...
  if (token.startsWith('sk-ant-sid')) {
    ...
    const orgUuid = process.env.TRASH_BOT_ORGANIZATION_UUID
    if (orgUuid) {
...
export function updateSessionIngressAuthToken(token: string): void {
  process.env.TRASH_BOT_SESSION_ACCESS_TOKEN = token
}
