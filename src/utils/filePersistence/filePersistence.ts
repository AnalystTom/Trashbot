 ...
 * - Checks environment kind (TRASH_BOT_ENVIRONMENT_KIND)
 * - Retrieves session access token
 * - Requires TRASH_BOT_REMOTE_SESSION_ID for session ID
 *
...
export async function runFilePersistence(
  ...
  const sessionId = process.env.TRASH_BOT_REMOTE_SESSION_ID
  if (!sessionId) {
    logError(
      new Error(
        'File persistence enabled but TRASH_BOT_REMOTE_SESSION_ID is not set',
      ),
    )
    return null
  }
