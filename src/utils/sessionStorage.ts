...
function getEntrypoint(): string | undefined {
  return process.env.TRASH_BOT_ENTRYPOINT
}
...
class Project {
   ...
   * TRASH_BOT_SKIP_PROMPT_HISTORY should suppress all transcript writes.
  ...
  private shouldSkipPersistence(): boolean {
    ...
    return (
      ...
      isSessionPersistenceDisabled() ||
      isEnvTruthy(process.env.TRASH_BOT_SKIP_PROMPT_HISTORY)
    )
  }
...
export async function loadTranscriptFile(
  ...
  try {
    ...
    if (!isEnvTruthy(process.env.TRASH_BOT_DISABLE_PRECOMPACT_SKIP)) {
    ...
    // TRASH_BOT_DISABLE_PRECOMPACT_SKIP is set (that kill switch means
    ...
    if (
      !opts?.keepAllLeaves &&
      !hasPreservedSegment &&
      !isEnvTruthy(process.env.TRASH_BOT_DISABLE_PRECOMPACT_SKIP) &&
...
export function isLoggableMessage(m: Message): boolean {
  ...
  if (m.type === 'attachment' && getUserType() !== 'ant') {
    if (
      ...
      isEnvTruthy(process.env.TRASH_BOT_SAVE_HOOK_ADDITIONAL_CONTEXT)
    ) {
      return true
    }
    return false
