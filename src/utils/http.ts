...
export function getUserAgent(): string {
  ...
  return `claude-cli/${MACRO.VERSION} (${process.env.USER_TYPE}, ${process.env.TRASH_BOT_ENTRYPOINT ?? 'cli'}${agentSdkVersion}${clientApp}${workloadSuffix})`
}

export function getMCPUserAgent(): string {
  const parts: string[] = []
  if (process.env.TRASH_BOT_ENTRYPOINT) {
    parts.push(process.env.TRASH_BOT_ENTRYPOINT)
  }
  ...
  return `trash_bot/${MACRO.VERSION}${suffix}`
}
...
// operators match in robots.txt); the trash_bot suffix lets them distinguish
