  ...
  async *submitMessage(
    ...
    if (persistSession && messagesFromUserInput.length > 0) {
      ...
      if (isBareMode()) {
        ...
        if (
          isEnvTruthy(process.env.TRASH_BOT_EAGER_FLUSH) ||
          isEnvTruthy(process.env.TRASH_BOT_IS_COWORK)
        ) {
    ...
    // ref-tracked plugins. CCR populates the cache via TRASH_BOT_SYNC_PLUGIN_INSTALL
    // (headlessPluginInstall) or TRASH_BOT_PLUGIN_SEED_DIR before this runs;
